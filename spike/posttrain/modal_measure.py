...
agent gets, and recorded in RESULTS.md.
    ...
    modal run spike/posttrain/modal_measure.py --track rm
    modal run spike/posttrain/modal_measure.py --track qa

Results land in spike/posttrain/results/{rm,qa}_anchors.json, which is what
...
def main(track: str = "rm", seeds: int = 5, arms: str = "", epochs: int = 0,
    ...
    # into spike/posttrain/results/, which is the directory every other script
    # reads.
    ...
    for arm, agg in summary["arms"].items():
        print(f"{arm:<14}" + "".join(
            ...
            for n in names))
    print(f"\nwrote {dest}")
    print(f"next: python spike/posttrain/finalize_anchors.py --track {track}")
