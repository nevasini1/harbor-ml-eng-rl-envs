...
#
#   ./scripts/regrade.sh jobs/2026-08-10__.../mol-property-adapt__abc123
...
#
# Differences from the protein task's regrade, both load-bearing:
#
#   * The agent's log is mounted too. task.toml ships /logs/agent/train_log.txt
#     as an artifact, and grade.py's contamination check scans it for held-out
#     molecules. Regrading without it would exercise less of the grader than
#     production does and could silently clear a submission that production
#     rejects.
#
#   * reward.json is single-key by design, and this task scores several eval
#     sets, so per-set AUC and recovery live in metrics.json. Both are printed:
#     the reward alone cannot tell you which eval set moved.
set -euo pipefail

TASK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "$TASK_DIR/../.." && pwd)"
IMAGE="${REGRADE_IMAGE:-mol-property-verifier:regrade}"
...
import json,sys
try:
    print(f\"{json.load(open(sys.argv[1]))['reward']:.4f}\")
except Exception:
    print('-')
" "$1" 2>/dev/null || echo "-"
}
...
if m.get('status') not in (None, 'ok'):
    print(f\"    grader: {m.get('status')} -- {str(m.get('reason'))[:80]}\")
for name, e in (m.get('eval_sets') or {}).items():
    st = e.get('status', '-')
    auc = e.get('auc')
    rec, raw = e.get('recovery'), e.get('recovery_raw')
    ov = e.get('private_test_overlap')
    bits = [f\"    {name:<8} {st:<9}\"]
    bits.append(f\"auc={auc:.4f}\" if isinstance(auc, float) else 'auc=-     ')
    ...
    if ov is not None:
        bits.append(f\"overlap={ov}\")
    if st not in ('ok',) and e.get('reason'):
        bits.append(f\"| {str(e['reason'])[:60]}\")
    print(' '.join(bits))
" "$1" 2>/dev/null || true
}
    ...
    case "$1" in
        --all)
            while IFS= read -r d; do TARGETS+=("$d"); done < <(
                find "$REPO_ROOT/jobs" -maxdepth 2 -type d -name 'mol-property-adapt__*' | sort
            )
            ;;
        --no-build) DO_BUILD=0 ;;
        -h|--help) usage ;;
